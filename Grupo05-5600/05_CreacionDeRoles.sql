...
GRANT EXECUTE ON LogicaBD.sp_PrecioDolarHoy TO rol_AdminGeneral;
GRANT EXECUTE ON LogicaBD.sp_PrecioDolarHoy TO rol_Sistemas;

USE master;
IF EXISTS (SELECT 1 FROM sys.server_principals WHERE name = 'lg_banco')
    DROP LOGIN lg_banco;
create login lg_banco with password = '123';
GRANT ADMINISTER BULK OPERATIONS TO lg_banco;

USE Com5600G05
-- Usuarios de ejemplo y asignacion a roles (contenidos en la BD)
IF EXISTS (SELECT 1 FROM sys.database_principals WHERE name = 'u_admin_general')
    DROP USER u_admin_general;
CREATE USER u_admin_general WITHOUT LOGIN;
IF EXISTS (SELECT 1 FROM sys.database_principals WHERE name = 'u_admin_bancario')
    DROP USER u_admin_bancario;
CREATE USER u_admin_bancario for LOGIN lg_banco;
IF EXISTS (SELECT 1 FROM sys.database_principals WHERE name = 'u_admin_operativo')
    DROP USER u_admin_operativo;
CREATE USER u_admin_operativo WITHOUT LOGIN;
IF EXISTS (SELECT 1 FROM sys.database_principals WHERE name = 'u_sistemas')
    DROP USER u_sistemas;
